...
class skels_dataset(Dataset):
    def __init__(self, path, patch_size=24, overlap=0.0, use_preprocess=False) -> None:
        super().__init__()
        ...
        self.fgp_in_patches = []
        img_name_list = sorted(os.listdir(os.path.join(path, 'img')))
        mask_name_list = sorted(os.listdir(os.path.join(path, 'mask')))

        self.mask_name_list = mask_name_list
        self.img_name_list = img_name_list

        for index, (img_name, mask_name) in tqdm(enumerate(zip(img_name_list, mask_name_list))):
            img = tiff.imread(os.path.join(path, 'img/', img_name))
            img = img.astype(np.float32)
            # preprocess image
            if use_preprocess:
                img = preprocess(img)
            mask = tiff.imread(os.path.join(path, 'mask/', mask_name))
            ...
            # add patches
            self.img_patches_list += img_patches
            self.mask_patches_list += mask_patches
        # indices of pathces
...
def get_dataset(args):
    train_dataset = skels_dataset(path=args.data, patch_size=args.patch_size, overlap=args.overlap)
...
if __name__ == "__main__":
    ...
    ds = skels_dataset('/share/home/liuy/project/seg_unet_cldice/data/skels_aug', patch_size=64, overlap=0)
    # img, mask = ds.__getitem__(np.random.randint(len(ds)))
    # arr_info(img, 'img')
    # arr_info(mask, 'mask')
    # tiff.imwrite('img.tif', tensor_to_ndarr(img[0]))
    # tiff.imwrite('mask.tif', tensor_to_ndarr(mask[0]))
